...
import { bestPath } from 'micro-web3/api/uniswap-v3'
...
export function UniswapAndSend(model: UniswapAndSendModel) {
	...
	const route = useSignal<ResolvePromise<ReturnType<typeof bestPath>> | undefined>(undefined)
	...
	const sourceChanged = () => {
		...
		queryTargetAmount(async function () {
			route.value = await bestPath(microWeb3Provider, tokensByName[sourceToken.peek()].address, tokensByName[targetToken.peek()].address, sourceAmount.peek(), undefined)
	...
	const targetChanged = () => {
		...
		querySourceAmount(async function () {
			route.value = await bestPath(microWeb3Provider, tokensByName[sourceToken.peek()].address, tokensByName[targetToken.peek()].address, undefined, targetAmount.peek())
	...
	const tokenChanged = () => (userSpecifiedValue.value === 'source') ? sourceChanged() : targetChanged()

	const SourceToken = (sourceAmountResult.value.state === 'pending') ? <Spinner/> : <TokenAndAmount key='source' token={sourceToken} amount={sourceAmount} onAmountChange={sourceChanged} onTokenChange={tokenChanged}/>
	const TargetToken = (targetAmountResult.value.state === 'pending') ? <Spinner/> : <TokenAndAmount key='target' token={targetToken} amount={targetAmount} onAmountChange={targetChanged} onTokenChange={tokenChanged}/>
	...
	function SubmitButton() {
		...
		// 	const swapTransactionDetails = txData(recipient.value, tokensByName[sourceToken.value].address, tokensByName[targetToken.value].address, route.value, route.value.amountIn, route.value.amountOut, { slippagePercent: 0.05, ttl: 60*60 })
