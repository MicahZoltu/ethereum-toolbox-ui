...
export function FixedPointInput(model: FixedPointInput) {
	const properties = {
		...
		tryParse: (input: string) => regexp.test(input) ? { ok: true, value: decimalStringToBigint(input, model.decimals.value) } as const : { ok: false } as const,
		serialize: (input: bigint) => input === 0n ? '' : bigintToDecimalString(input, model.decimals.value),
